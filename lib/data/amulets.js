...
exports.amuletTraitList = exports.amuletList = void 0;
exports.getAmulet = getAmulet;
exports.getAmuletTrait = getAmuletTrait;
exports.parseAmuletTraits = parseAmuletTraits;
exports.activeTraitIds = activeTraitIds;
exports.randomAmuletTrait = randomAmuletTrait;
exports.serializeAmuletTraits = serializeAmuletTraits;
exports.rollAmulet = rollAmulet;
exports.amuletList = [
...
const amuletById = new Map(exports.amuletList.map((item) => [item.id, item]));
const traitById = new Map(exports.amuletTraitList.map((item) => [item.id, item]));
function getAmulet(id) {
    return amuletById.get(id || 'prisoner-necklace');
}
function getAmuletTrait(id) {
    return traitById.get(id);
}
function parseAmuletTraits(value) {
    if (!value)
        return [];
    try {
...
function activeTraitIds(player) {
...
function randomAmuletTrait(random, excluded = []) {
    const pool = exports.amuletTraitList.filter((trait) => !excluded.includes(trait.id));
    return pool.length ? pool[Math.min(pool.length - 1, Math.floor(random() * pool.length))].id : undefined;
}
function serializeAmuletTraits(traits) {
    return JSON.stringify(traits);
}
function rollAmulet(random) {
    const roll = random();
    const traitCount = roll < 0.7 ? 1 : roll < 0.95 ? 2 : 3;
    const selected = [];
    while (selected.length < traitCount) {
        const rarity = random() < 0.7 ? 'common' : 'rare';
        const pool = exports.amuletTraitList.filter((trait) => trait.rarity === rarity && !selected.includes(trait.id));
