...
function addEvent(events, text, combatant) {
    events.push({ text, actorId: combatant?.userId, actorName: combatant?.username });
}
function traitIds(combatant) {
    return combatant.amuletTraits;
}
function hasTrait(combatant, id) {
    return traitIds(combatant).includes(id);
}
function traitEffect(combatant, effectId) {
    return traitIds(combatant).some((id) => (0, amulets_1.getAmuletTrait)(id)?.effectId === effectId);
}
function critDamageMultiplier(traits) {
    return 1 + traits.reduce((total, id) => total + ((0, amulets_1.getAmuletTrait)(id)?.critDamageBonus || 0), 0);
}
function thornRatio(traits) {
    return traits.reduce((ratio, id) => Math.max(ratio, (0, amulets_1.getAmuletTrait)(id)?.thornRatio || 0), 0);
}
function initiativeWeight(combatant) {
    const weight = combatant.amuletTraits
        .map(amulets_1.getAmuletTrait)
        .find((trait) => trait?.initiativeWeight)?.initiativeWeight;
    return weight || 50;
}
function greedMultiplier(combatant) {
    return combatant.amuletTraits
        .map(amulets_1.getAmuletTrait)
...
function createCombatant(player) {
    const stats = (0, progression_1.getPlayerStats)(player, true);
    const weapon = (0, equipment_1.getEquipment)(player.weaponId);
    const amuletTraits = (0, amulets_1.activeTraitIds)(player);
    return {
        ...
        prismMirrorChecked: false,
        starFuryChecked: false,
        frenziedFlameChecked: false,
        assassinationChecked: false,
        penNibReady: false,
        meteorFlash: amuletTraits.some((id) => (0, amulets_1.getAmuletTrait)(id)?.effectId === 'meteor-flash'),
        thornRatio: thornRatio(amuletTraits),
    };
}
function stateText(first, second) {
...
function applySnakeEye(combatant, random, events) {
    if (!traitEffect(combatant, 'snake-eye'))
        return;
...
function statusImmune(combatant, random) {
    return traitEffect(combatant, 'endurance') && percent(random, 50);
}
function addBleed(target, source, events, random) {
...
function applyStun(target, events, random, source, text) {
    if (statusImmune(target, random)) {
...
function applyFreeze(target, events, random, source, text = '冰冻') {
    if (statusImmune(target, random)) {
...
function heal(combatant, amount, events, label) {
...
function resetForRestart(target, opening) {
    Object.assign(target, cloneCombatant(opening));
}
function pickItem(combatant, random, forcedSlot) {
    const available = [
        !combatant.item1Used && combatant.item1Id ? { id: combatant.item1Id, slot: 1 } : undefined,
        !combatant.item2Used && combatant.item2Id ? { id: combatant.item2Id, slot: 2 } : undefined,
    ].filter(Boolean);
    if (forcedSlot)
        return available.find((item) => item.slot === forcedSlot);
    return available.length ? available[Math.min(available.length - 1, Math.floor(random() * available.length))] : undefined;
}
function hasAvailableItem(combatant) {
    return Boolean((combatant.item1Id && !combatant.item1Used) || (combatant.item2Id && !combatant.item2Used));
}
function useItem(combatant, defender, random, events, deal, forcedSlot) {
    const picked = pickItem(combatant, random, forcedSlot);
    if (!picked)
        return false;
    if (picked.slot === 1)
        combatant.item1Used = true;
    else
        combatant.item2Used = true;
        ...
        case 'vampirism-item':
            {
                ...
                if (dealt > 0)
                    heal(combatant, dealt, events, '吸血（道具）');
                return result;
            }
        case 'displacement':
...
function simulateBattle(first, second, config, random = Math.random) {
    ...
    for (const combatant of [left, right]) {
        if (traitEffect(combatant, 'offering')) {
            const damage = combatant.stats.maxHp * 0.99;
            combatant.hp = Math.max(1, combatant.hp - damage);
            combatant.consecutiveActions = 3;
            addEvent(events, `【祭品】${combatant.username} 失去 ${at(damage)} 点生命，并获得连续三回合行动！`, combatant);
        }
        if (traitEffect(combatant, 'orichalcum')) {
            combatant.barrierHp = combatant.stats.maxHp * 0.2;
            addEvent(events, `【奥利哈刚】${combatant.username} 获得 ${at(combatant.barrierHp)} 点护盾！`, combatant);
        }
    }
    ...
    let simultaneousWinnerId;
    addEvent(events, '已进入王座之间，准备决一死战！');
    addEvent(events, `${left.username}：HP ${left.stats.maxHp} | 攻击 ${left.stats.attack} |【${(0, equipment_1.getEquipment)(first.weaponId)?.name || '无'}】|【${(0, equipment_1.getEquipment)(first.shieldId)?.name || '无副手'}】`, left);
    addEvent(events, `${right.username}：HP ${right.stats.maxHp} | 攻击 ${right.stats.attack} |【${(0, equipment_1.getEquipment)(second.weaponId)?.name || '无'}】|【${(0, equipment_1.getEquipment)(second.shieldId)?.name || '无副手'}】`, right);
    addEvent(events, `🎲${attacker.username} 获得了先手！`, attacker);
    if (traitEffect(attacker, 'wind-shadow')) {
        if (percent(random, 1)) {
            defender.hp = 0;
            addEvent(events, `【风灵月影】${attacker.username} 直接取得胜利！`, attacker);
        }
        else {
            attacker.hp = 0;
            addEvent(events, `【风灵月影】${attacker.username} 直接死亡！`, attacker);
        }
    }
    const restart = () => {
    ...
    const takeDamage = (source, target, raw, options = {}) => {
        ...
        const targetShield = (0, equipment_1.getEquipment)(targetPlayer.shieldId);
        if (target.invincible) {
            target.invincible = false;
            addEvent(events, `${target.username} 的护盾效果使本次攻击无效！`, target);
            return 'survived';
        }
        if (!options.bypassBlock && options.weaponDamage !== false && targetShield?.blockRate && !(0, equipment_1.getEquipment)(sourcePlayer.weaponId)?.ignoreShield) {
            ...
            if (percent(random, blockRate)) {
                ...
                if ((0, equipment_1.getEquipment)(sourcePlayer.weaponId)?.effectId === 'blocked-crit') {
                ...
                if (targetShield.shieldEffect === 'stun')
                    applyStun(source, events, random, target, '击晕盾');
                if (targetShield.shieldEffect === 'freeze')
                    applyFreeze(source, events, random, target, '寒冰盾');
        ...
        if (!options.reflected && source !== target && traitEffect(target, 'scales') && damage > 50) {
            damage *= 0.5;
            addEvent(events, `${target.username} 的【鳞甲】使本次伤害降低50%！`, target);
        }
        if (damage > 0 && target.barrierHp > 0) {
            const absorbed = Math.min(damage, target.barrierHp);
            target.barrierHp -= absorbed;
            damage -= absorbed;
            addEvent(events, `${target.username} 的【奥利哈刚】吸收 ${at(absorbed)} 点伤害，剩余护盾 ${at(target.barrierHp)} 点！`, target);
            if (damage <= 0)
                return 'survived';
        }
        ...
        if (options.weaponDamage && !options.reflected && target.thornRatio > 0 && damage > 0) {
            ...
            if (reflected !== 'survived') {
                if (reflected === 'dead' && target.hp <= 0 && source.hp <= 0)
                    simultaneousWinnerId = source.userId;
                return reflected;
            }
        }
        ...
        if (source !== target && ((0, equipment_1.getEquipment)(targetPlayer.weaponId)?.effectId === 'retaliation-crit' || traitEffect(target, 'counter-stance'))) {
            target.retaliationReady = true;
            if (traitEffect(target, 'counter-stance'))
                addEvent(events, `${target.username} 的【反击架势】已准备，下一次主武器攻击必定暴击！`, target);
        }
        if (target.hp > 0)
            return 'survived';
    ...
    const hit = (source, target, attackPower, critical, damageDealtMultiplier, weaponDamage = true, useNextAttackMultiplier = false, allowTraitExtras = true) => {
        ...
        if (weaponDamage && allowTraitExtras && traitEffect(source, 'shinobi-execution') && !source.assassinationChecked && target.hp < 20) {
            source.assassinationChecked = true;
            target.hp = 0;
            addEvent(events, `${source.username} 的【忍杀】发动，处决了生命低于20的对手！`, source);
            return 'dead';
        }
        const penNibMultiplier = weaponDamage && source.penNibReady ? 2 : 1;
        if (weaponDamage && source.penNibReady) {
            source.penNibReady = false;
            addEvent(events, `${source.username} 的【笔尖】使本次主武器伤害翻倍！`, source);
        }
        const critMultiplier = critical ? 2 * source.extraCritDamageMultiplier : 1;
        const targetHpBefore = target.hp;
        const result = takeDamage(source, target, attackPower * critMultiplier * penNibMultiplier * (useNextAttackMultiplier ? source.nextAttackMultiplier : 1), { damageDealtMultiplier, critical, weaponDamage });
        const actualDamage = Math.max(0, targetHpBefore - target.hp);
        if (traitEffect(source, 'amulet-vampirism') && actualDamage > 0 && source.hp > 0) {
            heal(source, actualDamage * 0.5, events, '吸血（词条）');
        }
        if (result === 'restarted' || result === 'dead')
            return result;
        if (critical && source.extraCritDamageMultiplier !== critDamageMultiplier(source.amuletTraits))
            source.extraCritDamageMultiplier = critDamageMultiplier(source.amuletTraits);
        if (critical && traitEffect(target, 'war-cry'))
            target.extraCritDamageMultiplier = critDamageMultiplier(target.amuletTraits) * 2;
        if (critical && weaponDamage && allowTraitExtras && !source.starFuryChecked && traitEffect(source, 'starfury')) {
            source.starFuryChecked = true;
            const extraCrit = percent(random, Math.min(100, source.stats.critChance));
            addEvent(events, `${source.username} 的【星怒】追加一次独立攻击！${extraCrit ? '触发暴击！' : ''}`, source);
            const extra = hit(source, target, attackPower, extraCrit, damageDealtMultiplier, true, false, false);
            if (extra !== 'survived')
                return extra;
        }
        if (critical && allowTraitExtras && !source.frenziedFlameChecked && traitEffect(source, 'frenzied-flame') && percent(random, 25)) {
            source.frenziedFlameChecked = true;
            addEvent(events, `【癫火】爆发，${source.username} 与 ${target.username} 同时被眩晕！`, source);
            applyStun(source, events, random, source, '癫火');
            applyStun(target, events, random, source, '癫火');
        }
        return result;
    };
    let meteorWinnerId;
    if (left.meteorFlash || right.meteorFlash) {
        ...
        if (left.hp <= 0 && right.hp <= 0)
            meteorWinnerId = left.meteorFlash ? left.userId : right.userId;
    }
    const applyOngoing = (source, target) => {
    ...
    const action = (source, target) => {
        ...
        source.coldForgingUsed = false;
        source.prismMirrorChecked = false;
        source.starFuryChecked = false;
        source.frenziedFlameChecked = false;
        source.assassinationChecked = false;
        source.penNibReady = traitEffect(source, 'pen-nib') && source.actionCount % 3 === 0;
        if (traitEffect(source, 'golden-order'))
            heal(source, source.stats.maxHp * 0.15, events, '黄金律法');
        if (traitEffect(source, 'demon-form')) {
            source.stats.attack += 3;
            source.baseAttack += 3;
            addEvent(events, `${source.username} 的【恶魔形态】使攻击力提高3点！`, source);
        }
        ...
        const weapon = (0, equipment_1.getEquipment)(source === left ? first.weaponId : second.weaponId);
        ...
        if (mustCharge || firstSpearCharge) {
            if (mustCharge)
                source.heavyChargeUsed = true;
            if (firstSpearCharge)
                source.warSpearCharged = true;
        ...
        if (weapon?.effectId === 'preemptive-stun' && source !== (leftFirst ? left : right) && percent(random, 30)) {
            ...
            const result = hit(source, target, source.stats.attack + source.balancedBonus, preemptiveCritical, undefined, true, true);
            if (result !== 'survived')
                return result;
        ...
        if (source.actionCount === 1 && traitEffect(source, 'bottled-lightning') && source.item1Id && !source.item1Used) {
            const used = useItem(source, target, random, events, (raw, critical = false) => hit(source, target, raw, critical, undefined, false), 1);
            if (used !== false)
                return used;
        }
        const offhand = (0, equipment_1.getEquipment)(source === left ? first.shieldId : second.shieldId);
        if (!offhandCooling && offhand?.type === 'offhand') {
            let useOffhand = false;
            if (offhand.effectId === 'ice-bow' || offhand.effectId === 'bleed')
                useOffhand = true;
        ...
        if (weapon?.guaranteedCritAttacks?.includes(source.attackCount))
            guaranteedCrit = true;
        if (weapon?.lowHpCrit && source.hp < source.stats.maxHp * 0.5)
            guaranteedCrit = true;
        if (weapon?.frozenTargetCrit && target.frozen)
            guaranteedCrit = true;
        ...
        if (weapon?.effectId === 'first-turn-crit' && turns === 1)
            guaranteedCrit = true;
        if (weapon?.effectId === 'lightning-beam' && turns >= 2)
            guaranteedCrit = true;
        if (weapon?.effectId === 'retaliation-crit' && source.retaliationReady)
            guaranteedCrit = true;
        ...
        for (let index = 0; index < hitCount; index++) {
            ...
            result = hit(source, target, attackPower, separateCritical, attackDamageMultiplier, true, true);
            if (result !== 'survived')
                return result;
            if (weapon?.effectId === 'blood-blade' || weapon?.effectId === 'bleed')
                addBleed(target, source, events, random);
            if (weapon?.stunRate && percent(random, weapon.stunRate))
                applyStun(target, events, random, source, '胡桃夹子');
        }
        if (traitEffect(source, 'prism-mirror') && !source.prismMirrorChecked && percent(random, 30)) {
            source.prismMirrorChecked = true;
            const extraCrit = percent(random, Math.min(100, critChance));
            addEvent(events, `${source.username} 的【棱镜之镜】追加一次独立攻击！${extraCrit ? '触发暴击！' : ''}`, source);
            result = hit(source, target, attackPower, extraCrit, attackDamageMultiplier, true, true, false);
            if (result !== 'survived')
                return result;
        }
        if (weapon?.effectId === 'electric-damage') {
            ...
            for (let index = 0; index < extraHits; index++) {
                result = hit(source, target, attackPower, critical, attackDamageMultiplier, true, true);
                if (result !== 'survived')
                    return result;
            }
        }
        ...
        if (weapon?.effectId === 'heavy-charge')
            source.heavyChargeUsed = false;
        ...
        if (traitEffect(source, 'combo')) {
            source.stats.attack += 5;
            source.baseAttack += 5;
            addEvent(events, `${source.username} 的【连击】使主武器攻击力提高5点！`, source);
        }
        if (source.nextAttackMultiplier !== 1)
            source.nextAttackMultiplier = 1;
        return 'survived';
    };
    while (left.hp > 0 && right.hp > 0 && turns < config.maxBattleTurns) {
        ...
        if (left.hp <= 0 || right.hp <= 0)
            break;
        if (attacker.frontlineTurns > 0)
            attacker.frontlineTurns--;
        ...
        if (attacker.consecutiveActions > 0)
            attacker.consecutiveActions--;
        if (attacker.consecutiveActions === 0) {
            ;
    ...
    const winner = meteorWinnerId === left.userId
        ? left
        : meteorWinnerId === right.userId
            ? right
            : simultaneousWinnerId === left.userId
                ? left
                : simultaneousWinnerId === right.userId
                    ? right
                    : left.hp > 0 ? left : right;
    ...
    if (winner.hunterReady) {
        ...
        const candidates = [];
        if (loserPlayer.weaponId)
            candidates.push({
                type: 'weapon',
                id: loserPlayer.weaponId,
                weaponQuality: loserPlayer.weaponQuality,
                weaponTrait: loserPlayer.weaponTrait,
            });
        if (loserPlayer.shieldId)
            candidates.push({ type: 'offhand', id: loserPlayer.shieldId });
        if (loserPlayer.item1Id)
            candidates.push({ type: 'item', id: loserPlayer.item1Id });
        if (loserPlayer.item2Id)
            candidates.push({ type: 'item', id: loserPlayer.item2Id });
        if (loserPlayer.amuletId)
            candidates.push({ type: 'amulet', id: loserPlayer.amuletId, traits: (0, amulets_1.parseAmuletTraits)(loserPlayer.amuletTraits) });
        if (candidates.length)
            droppedEquipment = candidates[Math.floor(random() * candidates.length)];
    }
    return {
        attacker: left,
        defender: right,
