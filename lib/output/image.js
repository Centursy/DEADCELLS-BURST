...
function equipmentPanel(label, id, slot) {
...
async function renderBossTraitChoiceCard(ctx, player, boss, choices, rewardCells, timeoutSeconds = 30) {
    ...
    <div class="boss-choice-card"><div class="boss-choice-title">最后一击奖励</div><div class="boss-choice-subtitle">从以下五个不重复词条中选择一个；已有两个词条时还需选择替换槽位。</div><div class="boss-choice-list">${choices.map((id, index) => { const trait = (0, amulets_1.getAmuletTrait)(id); return `<div class="boss-choice-item"><div class="boss-choice-number">${index + 1}</div><div><div class="boss-choice-name">${escapeHtml(trait?.name || id)}</div><div class="boss-choice-description">${escapeHtml(trait?.description || '')}</div></div></div>`; }).join('')}</div></div>
