...
function createCombatant(player) {
    ...
    const amuletTraits = (0, amulets_1.parseAmuletTraits)(player.amuletTraits);
    return {
...
function pickItem(combatant, random) {
    const available = [
...
function useItem(combatant, defender, random, events, deal) {
...
function simulateBattle(first, second, config, random = Math.random) {
    ...
    const takeDamage = (source, target, raw, options = {}) => {
        ...
        if ((0, equipment_1.getEquipment)(targetPlayer.weaponId)?.effectId === 'retaliation-crit')
    ...
    const hit = (source, target, attackPower, critical, damageDealtMultiplier, weaponDamage = true) => {
        ...
        if (traitEffect(source, 'amulet-vampirism'))
            heal(source, attackPower * critMultiplier * 0.5, events, '吸血（词条）');
        return result;
    };
    let meteorWinnerId;
    ...
    if (winner.hunterReady) {
        ...
        if (loserPlayer.weaponId)
            candidates.push({ type: 'weapon', id: loserPlayer.weaponId });
