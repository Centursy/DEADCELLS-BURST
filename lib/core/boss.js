...
function bossConfig(difficulty, random) {
    if (difficulty === 'veteran')
        return { maxHp: randomInt(random, 46000, 62000), attackMultiplier: 1.5, rewardMultiplier: 2 };
    if (difficulty === 'veteran-king')
        return { maxHp: randomInt(random, 70000, 100000), attackMultiplier: 2, rewardMultiplier: 3 };
...
function calculateBossReward(player, damage, rewardMultiplier) {
    ...
    return Math.round(damage * 10 * rewardMultiplier * levelMultiplier * greedMultiplier((0, amulets_1.parseAmuletTraits)(player.amuletTraits)));
}
...
function randomTraitChoices(random, count = 5) {
    const pool = [...amulets_1.amuletTraitList];
    const choices = [];
...
function simulateBossRaid(player, boss, config, random = Math.random) {
    ...
    const events = [];
    const deal = (raw, forcedCrit = false) => {
        if (remaining <= 0)
            return;
    ...
    while (playerHp > 0 && remaining > 0 && turns < config.maxBattleTurns) {
        ...
        if (useItem) {
            const item = (0, equipment_1.getEquipment)(player.item1Id || player.item2Id);
            if (item?.effectId === 'powerful-grenade')
                deal(stats.attack * 1.5);
            ...
            for (let index = 0; index < hits && remaining > 0; index++)
                deal(stats.attack, guaranteedCrit);
