...
function rankingForUser(rankings, userId, username, damage) {
...
async function settleBossChoice(ctx, config, session, player, state, firstAnswer) {
    ...
    if (currentTraits.length >= 2) {
        ...
        if (!['1', '2'].includes(slotAnswer.trim()))
            return '请选择 1 或 2 替换护符词条。';
    ...
    return `已获得 ${state.rewardCells} 个细胞，并装备词条【${trait?.name || selectedTrait}】。${currentTraits.length >= 2 ? '已替换选择的词条槽位。' : currentTraits.length === 1 ? '已装入第二个词条槽位。' : `已获得护符【${(0, amulets_1.getAmulet)(amuletId)?.name || '炼化护符'}】。`}`;
}
function registerBossCommand(ctx, config, busy) {
    ...
    ctx.command(`${commandName} [choice:text]`)
        ...
        try {
            ...
            const rankings = rankingForUser((0, boss_1.parseBossRankings)(boss.rankings), player.userId, player.username, result.damage);
            ...
            const reward = (0, boss_1.calculateBossReward)(player, result.damage, boss.rewardMultiplier);
            if (killed) {
                const state = {
                    date: boss.date,
