...
import { raidBossMaps } from './boss'
import type { BattleEvent, GameConfig, DeathmatchResult, DeadcellsPlayer, Random } from '../types'

type DmResult = 'survived' | 'dead'
...
interface DmFighter {
  ...
  item1Id: string | null
  item2Id: string | null
  stats: { maxHp: number; attack: number; critChance: number }
  hp: number
  maxHp: number
  attackCount: number
  actionCount: number
...
function addEvent(events: BattleEvent[], text: string, actor?: DmFighter): void {
  events.push({ text, actorId: actor?.userId, actorName: actor?.username })
}

function playerStatsForDeathmatch(player: DeadcellsPlayer) {
  return getPlayerStats({ ...player, amuletTraits: '[]', weaponTrait: null })
}

function createFighter(player: DeadcellsPlayer): DmFighter {
...
function createBoss(name: string, hp: number): DmFighter {
  return {
...
function alive(fighter: DmFighter): boolean {
  return fighter.hp > 0
}

function weapon(source: DmFighter) {
  return getEquipment(source.weaponId)
}

function offhand(source: DmFighter) {
  return getEquipment(source.shieldId)
}

function itemAvailable(source: DmFighter, random: Random): { id: string; slot: 1 | 2 } | undefined {
  const choices = [
    source.item1Id && !source.item1Used ? { id: source.item1Id, slot: 1 as const } : undefined,
    source.item2Id && !source.item2Used ? { id: source.item2Id, slot: 2 as const } : undefined,
...
function heal(source: DmFighter, amount: number, events: BattleEvent[], label: string): void {
...
function addBleed(target: DmFighter, source: DmFighter, events: BattleEvent[]): void {
  target.bleeding = 3
...
function applyStatus(target: DmFighter, type: 'stun' | 'freeze', events: BattleEvent[], source: DmFighter, label: string): void {
...
function takeDamage(
  ...
  random: Random,
  events: BattleEvent[],
  options: { weaponDamage?: boolean; critical?: boolean; nonLethal?: boolean; reflected?: boolean; fixedDamage?: number } = {},
): DmResult {
  if (!alive(target)) return 'dead'
  if (target.decoyActive && !options.reflected) {
    ...
    if (target.decoyHp > 0) return 'survived'
  ...
  if (targetShield && shieldBlocked && sourceWeapon?.effectId === 'blocked-crit') {
    ...
    if (targetShield.shieldEffect === 'stun') applyStatus(source, 'stun', events, target, '击晕盾')
    if (targetShield.shieldEffect === 'freeze') applyStatus(source, 'freeze', events, target, '寒冰盾')
    ...
    if (targetShield.shieldEffect === 'invincible') addEvent(events, `${target.username} 获得下一次受击无敌！`, target)
...
function pickTarget(source: DmFighter, candidates: DmFighter[], random: Random): DmFighter | undefined {
...
function useItem(source: DmFighter, target: DmFighter, random: Random, events: BattleEvent[], config: GameConfig): DmResult | false {
  const choices = [
    source.item1Id && !source.item1Used ? { id: source.item1Id, slot: 1 as const } : undefined,
    source.item2Id && !source.item2Used ? { id: source.item2Id, slot: 2 as const } : undefined,
  ].filter(Boolean) as Array<{ id: string; slot: 1 | 2 }>
  ...
  if (picked.slot === 1) source.item1Used = true
  else source.item2Used = true
  const item = getEquipment(picked.id)
  if (!item) return false
  addEvent(events, `${source.username} 使用道具【${item.name}】，本回合不进行主武器攻击！`, source)
  if (item.effectId === 'powerful-grenade') return takeDamage(source, target, source.stats.attack * 1.5, random, events)
  if (item.effectId === 'cluster-grenade') {
    ...
    if (item.effectId === 'serenade') source.nightSongActive = true
...
function tickOngoing(source: DmFighter, target: DmFighter | undefined, fighters: DmFighter[], random: Random, events: BattleEvent[], config: GameConfig): DmResult {
  ...
  if (target && source.turretEffect && source.turretTurns > 0) {
    ...
    source.turretTurns--
    if (result === 'dead') return result
    if (source.turretEffect === 'circular') addBleed(target, source, events)
  }
...
function playerAction(source: DmFighter, target: DmFighter, fighters: DmFighter[], random: Random, events: BattleEvent[], config: GameConfig): DmResult {
  source.actionCount++
  if (source.offhandCooldown > 0) source.offhandCooldown--
  if (tickOngoing(source, target, fighters, random, events, config) === 'dead') return 'dead'
  ...
  if (source.offhandCooldown === 0 && currentOffhand?.type === 'offhand' && (currentOffhand.effectId === 'ice-bow' || currentOffhand.effectId === 'bleed' || currentOffhand.skill === 'north-star') && percent(random, config.skillRate)) {
    ...
    } else if (currentOffhand.effectId === 'bleed') addBleed(target, source, events)
    else {
  ...
  if (currentWeapon?.guaranteedCritAttacks?.includes(source.attackCount)) guaranteedCrit = true
  if (currentWeapon?.lowHpCrit && source.hp < source.maxHp * 0.5) guaranteedCrit = true
  if (currentWeapon?.frozenTargetCrit && target.frozen) guaranteedCrit = true
  if (currentWeapon?.effectId === 'first-turn-crit' && source.attackCount === 1) guaranteedCrit = true
  if (currentWeapon?.effectId === 'even-crit' && source.actionCount % 2 === 0) guaranteedCrit = true
  if (currentWeapon?.effectId === 'third-turn-crit' && source.actionCount === 3) guaranteedCrit = true
  if (currentWeapon?.effectId === 'lightning-beam' && source.actionCount >= 2) guaranteedCrit = true
  if (currentWeapon?.skill && percent(random, config.skillRate)) guaranteedCrit = true
  if (source.charging) guaranteedCrit = true
  ...
  if (currentWeapon?.effectId === 'bleed' && alive(target)) addBleed(target, source, events)
...
function bossAction(boss: DmFighter, target: DmFighter, fighters: DmFighter[], random: Random, events: BattleEvent[], config: GameConfig): DmResult {
  const ongoing = tickOngoing(boss, target, [boss, ...fighters], random, events, config)
  if (ongoing === 'dead') return ongoing
...
function randomBoss(random: Random): { mapName: string; bossName: string } {
...
export function simulateDeathmatch(players: DeadcellsPlayer[], config: GameConfig, random: Random = Math.random): DeathmatchResult {
  ...
  let turns = 0
  let lastBossKillerId: string | undefined
  ...
  while (alive(boss) && fighters.some(alive) && turns < config.maxBattleTurns * Math.max(1, fighters.length)) {
    for (const fighter of fighters) {
      if (!alive(fighter) || !alive(boss)) continue
      turns++
      const before = boss.hp
      const result = playerAction(fighter, boss, fighters, random, events, config)
      if (before > 0 && boss.hp <= 0) lastBossKillerId = fighter.userId
      if (result === 'dead') addEvent(events, `${fighter.username} 被 Boss 击败！`, fighter)
      if (!alive(boss)) break
    }
    if (alive(boss)) {
      ...
      if (result === 'dead' && bleedSourceId) lastBossKillerId = bleedSourceId
      if (result === 'dead') addEvent(events, `${target.username} 被 Boss 击败！`, target)
    }
  }

  const bossKilled = !alive(boss)
  const bossReward = bossKilled ? bossHp * 100 : 0
  let winner: DmFighter | undefined
  if (bossKilled) {
    ...
    if (alivePlayers.length > 0) {
      ...
      while (!winner && round < config.maxBattleTurns * Math.max(1, fighters.length)) {
        ...
        for (const fighter of order) {
          if (!alive(fighter) || winner) continue
          ...
          if (result === 'dead') addEvent(events, `${target.username} 被 ${fighter.username} 淘汰！`, fighter)
          if (fighters.filter(alive).length === 1) winner = fighters.find(alive)
        }
      }
    }
  }
