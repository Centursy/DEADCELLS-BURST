...
const mapBosses: Record<string, { name: string; image: string }> = {
...
function assetData(fileName: string | undefined): string | undefined {
...
async function avatarData(userId: string): Promise<string | undefined> {
  ...
  try {
    ...
    if (!response.ok || !mime?.startsWith('image/')) throw new Error(`HTTP ${response.status}`)
    const buffer = Buffer.from(await response.arrayBuffer())
    if (buffer.length === 0 || buffer.length > 4 * 1024 * 1024) throw new Error('头像文件大小异常')
...
function avatarTag(source: string | undefined, username: string, className = 'avatar'): string {
  if (source) return `<img class="${className}" src="${escapeHtml(source)}" alt="" />`
  const initial = escapeHtml(username.trim().slice(0, 1) || '?')
  return `<div class="${className} avatar-fallback">${initial}</div>`
}

function imageTag(id: string | null | undefined, className = 'equipment-image'): string {
...
function amuletTraitText(player: DeadcellsPlayer): string {
...
function bossCellTag(level: number): string {
...
function amuletPanel(player: DeadcellsPlayer): string {
...
async function renderCard(ctx: Context, html: string): Promise<any | undefined> {
...
function pageHtml(content: string, cardClass = 'card'): string {
...
export async function renderPlayerCard(ctx: Context, player: DeadcellsPlayer): Promise<any | undefined> {
  ...
  const html = pageHtml(`
    ...
    <div class="footer"><span>BATTLES ${player.battleCount}</span><span>WIN RATE ${formatWinRate(player)}</span><span>BURST // 0.1</span></div>
  `)
  return renderCard(ctx, html)
}

function routeNode(mapName: string, index: number, currentMap: string): string {
...
function bossCard(encounter: ExplorationResult['bosses'][number], result: ExplorationResult): string {
...
async function renderCompletionCard(
  ctx: Context,
...
export async function renderExploreCard(
  ctx: Context,
  player: DeadcellsPlayer,
  result: ExplorationResult,
): Promise<any | undefined> {
  if (result.completed) return renderCompletionCard(ctx, player, result)
  const avatar = await avatarData(player.userId)
  const currentMap = result.reached[result.reached.length - 1] || maps[0].name
...
function fighterCard(player: DeadcellsPlayer, hp: number, maxHp: number, winner: boolean, avatar: string | undefined): string {
  const stats = getPlayerStats(player)
...
function logClass(text: string): string {
  ...
  if (/暴击|即死/.test(text)) classes.push('critical')
  if (/技能|发动|触发|标记|蓄力|箭塔|召唤/.test(text)) classes.push('skill')
  if (/格挡|护盾|无敌|反弹|抵挡|保留1点生命|免疫/.test(text)) classes.push('defense')
  if (/眩晕|冰冻|流血|充能|沉默/.test(text)) classes.push('status')
  if (/受到\s*\d+\s*点伤害|死亡|战败|击败/.test(text)) classes.push('danger')
  if (/获胜|胜利|WINS|战斗结束/.test(text)) classes.push('victory')
  if (/使用道具|使用副手/.test(text)) classes.push('item')
  return classes.join(' ')
}

function hpBar(name: string, hp: number, maxHp: number): string {
...
function battleLogLine(text: string, firstName?: string, secondName?: string): string {
...
export async function renderBattleCard(
  ctx: Context,
...
export async function renderAmuletCard(
  ctx: Context,
  player: DeadcellsPlayer,
  generated: { id: string; traits: string[] },
  cost = 3000,
): Promise<any | undefined> {
  const avatar = await avatarData(player.userId)
  const amulet = getAmulet(generated.id)
  const traits = generated.traits.map(getAmuletTrait).filter(Boolean)
  const html = pageHtml(`
    <div class="header battle-header">
...
export async function renderAmuletChoicesCard(
  ctx: Context,
  player: DeadcellsPlayer,
  generated: Array<{ id: string; traits: string[] }>,
  cost: number,
): Promise<any | undefined> {
  const avatar = await avatarData(player.userId)
  const html = pageHtml(`
    ...
    <div class="amulet-choice-grid">${generated.map((item, index) => {
      const amulet = getAmulet(item.id)
      const traits = item.traits.map(getAmuletTrait).filter(Boolean)
...
export async function renderAmuletDropCard(
  ctx: Context,
  winner: DeadcellsPlayer,
  amuletId: string,
  traits: string[],
  ...
  const avatar = await avatarData(winner.userId)
  const amulet = getAmulet(amuletId)
  const traitDefinitions = traits.map(getAmuletTrait).filter(Boolean)
...
function bossDifficultyName(difficulty: DailyBossRecord['difficulty']): string {
  return difficulty === 'veteran-king' ? '历战王' : difficulty === 'veteran' ? '历战' : '普通'
}

export async function renderBossRaidCard(
  ctx: Context,
  player: DeadcellsPlayer,
  boss: DailyBossRecord,
  result: BossRaidResult | undefined,
  rankings: Array<{ userId: string; username: string; damage: number }>,
...
export async function renderBossTraitChoiceCard(
  ctx: Context,
  player: DeadcellsPlayer,
  boss: DailyBossRecord,
  choices: string[],
