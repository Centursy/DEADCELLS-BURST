import { activeTraitIds, getAmuletTrait, parseAmuletTraits } from '../data/amulets'
...
function createCombatant(player: DeadcellsPlayer): Combatant {
  ...
  const amuletTraits = activeTraitIds(player)
  return {
...
function pickItem(combatant: Combatant, random: Random, forcedSlot?: 1 | 2): { id: string; slot: 1 | 2 } | undefined {
  const available = [
  ...
  if (forcedSlot) return available.find((item) => item.slot === forcedSlot)
...
export function simulateBattle(
  ...
  const takeDamage = (source: Combatant, target: Combatant, raw: number, options: { bypassDecoy?: boolean; bypassBlock?: boolean; nonLethal?: boolean; damageDealtMultiplier?: number; fixedDamage?: number; critical?: boolean; weaponDamage?: boolean; reflected?: boolean } = {}): DamageResult => {
    ...
    if (options.weaponDamage && !options.reflected && target.thornRatio > 0 && damage > 0) {
      ...
      if (reflected !== 'survived') {
        if (reflected === 'dead' && target.hp <= 0 && source.hp <= 0) simultaneousWinnerId = source.userId
        return reflected
      }
    }
    ...
    if (source !== target && (getEquipment(targetPlayer.weaponId)?.effectId === 'retaliation-crit' || traitEffect(target, 'counter-stance'))) {
      target.retaliationReady = true
      if (traitEffect(target, 'counter-stance')) addEvent(events, `${target.username} 的【反击架势】已准备，下一次主武器攻击必定暴击！`, target)
    }
  ...
  const hit = (source: Combatant, target: Combatant, attackPower: number, critical: boolean, damageDealtMultiplier?: number, weaponDamage = true, useNextAttackMultiplier = false, allowTraitExtras = true): DamageResult => {
    ...
    if (critical && weaponDamage && allowTraitExtras && !source.starFuryChecked && traitEffect(source, 'starfury')) {
      ...
      if (extra !== 'survived') return extra
    }
  ...
  const action = (source: Combatant, target: Combatant): DamageResult => {
    ...
    if (traitEffect(source, 'golden-order')) heal(source, source.stats.maxHp * 0.15, events, '黄金律法')
  ...
  while (left.hp > 0 && right.hp > 0 && turns < config.maxBattleTurns) {
    ...
    if (attacker.consecutiveActions > 0) attacker.consecutiveActions--
  ...
  if (winner.hunterReady) {
    ...
    if (loserPlayer.weaponId) candidates.push({
      type: 'weapon',
