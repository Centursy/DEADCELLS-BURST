...
function playerStatus(player) {
    ...
    const weaponTrait = player.weaponQuality === 'colorless' && player.weaponTrait ? (0, amulets_1.getAmuletTrait)(player.weaponTrait) : undefined;
    return [
        `${player.username}`,
        `细胞数：${player.cells}`,
        `【${player.bossCellLevel}】细胞等级`,
        `HP ${stats.maxHp} | 攻击 ${stats.attack} | 暴击 ${stats.critChance}%`,
        `武器：【${(0, equipment_1.weaponQualityText)(player.weaponQuality)}·${stats.weaponName}】${(0, equipment_1.weaponQualityEffectText)(player.weaponQuality) ? `|${(0, equipment_1.weaponQualityEffectText)(player.weaponQuality)}` : ''}${weaponTrait ? `|无色词条：${weaponTrait.name}` : ''}|副手：【${stats.shieldName}】`,
...
function equipmentDescription(player) {
    ...
    return [
        '当前装备',
        `武器：【${(0, equipment_1.weaponQualityText)(player.weaponQuality)}·${weapon?.name || '无'}】${weapon ? `：${[weapon.description, (0, equipment_1.weaponQualityEffectText)(player.weaponQuality), weaponTrait ? `无色词条：${weaponTrait.name}（${weaponTrait.description}）` : ''].filter(Boolean).join('｜')}` : ''}`,
