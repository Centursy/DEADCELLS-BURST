    ...
    ctx.command(`${config.commandDuel} <target:text>`)
        ...
        try {
            ...
            const equipment = copied && copied.type !== 'amulet'
                ? (0, equipment_1.getEquipment)(copied.id)
                : !copied && Math.random() * 100 < config.equipmentDropRate
                    ? (0, equipment_1.randomEquipment)(Math.random)
                    : undefined;
            ...
            if (equipment) {
                ...
                if (equipmentCard) {
                    ...
                    await session.send(`【装备获取】恭喜 ${koishi_1.h.at(winner.userId)} 获得装备【${equipment.name}】！\n${equipment.description}\n${autoEquipped ? '已自动装备。' : prompt || '回复 y 替换当前装备，其他内容或超时放弃。'}`);
                }
                if (autoEquipped)
                    return;
                let field;
