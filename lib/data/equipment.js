...
exports.equipmentById = exports.equipmentList = void 0;
exports.getEquipment = getEquipment;
exports.randomEquipment = randomEquipment;
exports.randomEquipmentChoices = randomEquipmentChoices;
exports.randomWeaponQuality = randomWeaponQuality;
exports.weaponQualityText = weaponQualityText;
exports.weaponQualityBonus = weaponQualityBonus;
exports.weaponQualityEffectText = weaponQualityEffectText;
exports.createEquipmentReward = createEquipmentReward;
exports.equipmentTypeText = equipmentTypeText;
const amulets_1 = require("./amulets");
exports.equipmentList = [
...
exports.equipmentById = new Map(exports.equipmentList.map((equipment) => [equipment.id, equipment]));
function getEquipment(id) {
    return id ? exports.equipmentById.get(id) : undefined;
}
function randomEquipment(random, type) {
    const pool = type ? exports.equipmentList.filter((equipment) => equipment.type === type) : exports.equipmentList;
    return pool[Math.min(pool.length - 1, Math.floor(random() * pool.length))];
}
function randomEquipmentChoices(random, count = 3) {
    const pool = [...exports.equipmentList];
    const result = [];
...
function randomWeaponQuality(random) {
    const roll = random();
    return roll < 0.75 ? 'normal' : roll < 0.95 ? 'gold' : 'colorless';
}
function weaponQualityText(quality) {
    return quality === 'gold' ? '金色' : quality === 'colorless' ? '无色' : '普通';
}
function weaponQualityBonus(quality) {
    return quality === 'gold' ? { attack: 5, crit: 5 } : { attack: 0, crit: 0 };
}
function weaponQualityEffectText(quality) {
    if (quality === 'gold')
        return '金色加成：攻击力+5，暴击率+5%';
    if (quality === 'colorless')
        return '无色品质：附带一个护符词条';
    return '';
}
function createEquipmentReward(equipment, random, excludedTraits = []) {
    ...
    return {
        ...equipment,
        weaponQuality,
        weaponTrait: weaponQuality === 'colorless' ? (0, amulets_1.randomAmuletTrait)(random, excludedTraits) || null : null,
    };
}
